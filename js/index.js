//0) Un poco de manipulación de DOM
const texto = document.getElementById("dom");
texto.textContent = "Para ver un poco de manipulación de dom, dirigirse a la sección The Graves";
...
function seleccionarImagenes() {
        ...
        // Verificar si la selección es null
        if (seleccion === null) {
        return;
        }
...
//6) FUNCIÓN PARA RESTAURAR EL STOCK SI EL USUARIO DESISTE DE LA COMPRA
function restaurarStock(){
    carrito.forEach(producto => {
        const productoCatalogo = catalogo.find(item => item.personaje === producto.personaje);
        productoCatalogo.stock += producto.cantidad;
    });
}

//7) FUNCIÓN PARA SALIR DEL PROGRAMA O VOLVER AL MENÚ
const volverSalir = () =>{
    let respuesta;
     do{
